...
if (file.exists("data/polyhedra.RDS")) {
  polyhedra.saved <- readRDS("data/polyhedra.RDS")
}
for (polyhedra.file in polyhedra.files) {
  ...
  current.polyedron <- Polyhedron.class$new(polyhedra.file)
  self <- current.polyedron$state
  current.polyedron$scrape(netlib.p3.lines = polyedra.netlib.lines)
  futile.logger::flog.debug(paste("parsed", polyhedra.file, "with name", current.polyedron$state$name))
  polyhedra[[polyhedra.file]] <- current.polyedron
}
```
